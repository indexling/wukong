...
<link href="{{asset('/styles/css/style.css')}}" type="text/css" rel="stylesheet">
<link href="{{asset('/styles/css/external.min.css')}}" type="text/css" rel="stylesheet">
<link href="{{asset('/styles/css/popup.css')}}" type="text/css" rel="stylesheet">
<script type="text/javascript" src="{{asset('/styles/js/jquery.1.10.1.min.js')}}"></script>
<script src="{{asset('/styles/js/jquery.lib.min.js')}}" type="text/javascript"></script>
<script type="text/javascript" src="{{asset('/styles/js/ajaxfileupload.js')}}"></script>

<script type="text/javascript">
</script> 
<script src="{{asset('/styles/js/ajaxCross.json')}}" charset="UTF-8"></script></head>
<body>
<div id="body">
	<div id="header">
    	<div class="wrapper">
    		<a class="logo" href="h/">
    			<img width="229" height="43" alt="拉勾招聘-专注互联网招聘" src="/styles/images/logo.png">
    		</a>
            	...
            	<div class="profile_box" id="basicInfo">
            		...
            		<div class="basicShow">
            			...
            			<div class="m_portrait">
	                    	<div></div>
	                    	<img width="120" height="120" alt="jason" src="{{asset('/styles/images/default_headpic.png')}}">
	                    </div>
            		</div><!--end .basicShow-->
            			...
            			<form id="profileForm">
						  <table>
						    ...
						    <tr>
						      <td></td> 
						      <td colspan="3">
						          <input type="submit" id="" value="保 存" class="btn_profile_save">
            	...
            	<div class="profile_box" id="expectJob">
            		...
            		<div class="expectEdit dn">
            			<form id="expectForm">
	            			<table>
	            				<tbody><tr>
	            					...
	            					<td>
	            						<ul class="profile_radio clearfix reset">
	            								            								<li class="current">
									             	 全职<em></em>
									              	<input type="radio" checked="" name="type" value="2"> 
									            </li>
									            <li>
									              	兼职<em></em>
	            				...
	            				<tr>
	            					<td colspan="2">
										<input type="button" id="expectsub" value="保 存" class="btn_profile_save">
						          		<a class="btn_profile_cancel" href="javascript:;">取 消</a>
	            					</td>
	            				</tr>
	            			</tbody></table>
					...
					//期望工作提交
						...
						$.ajax({
							type: 'POST',
							url: '/ajax/expect',
							data: { expectCity : expectCity,type:type,expectPosition:expectPosition,expectSalary:expectSalary},
							dataType: 'json',
							headers: {
							...
							console.log(data);
					    		...
					    		}else{alert('请输入期望职位')}}else{alert('请选择城市')}
            			...
            			<form class="experienceForm">
	            			<table>
	            				...
	            				<tr>
	            					<td></td>
	            					<td colspan="3">
										<input type="button" value="保 存" id='btn_profile_save' class="btn_profile_save">
						          		<a class="btn_profile_cancel" href="javascript:;">取 消</a>
	            					</td>
	            				</tr>
	            			</tbody></table>
...
$(function(){
	...
	$('#btn_profile_save').click(function(){
		...
		var companyYearStart = $("input[name='companyYearStart']").val();
		var companyMonthStart = $("input[name='companyMonthStart']").val();
		var companyYearEnd = $("input[name='companyYearEnd']").val();
		var companyMonthEnd = $("input[name='companyMonthEnd']").val();


// console.log(isdelta(2,1,2,2));
		if (companyName&&positionName&&companyYearStart&&companyMonthStart&&companyYearEnd&&companyMonthEnd) {
			//数据正确性验证 return int/false
			if ( isdelta(companyYearStart,companyMonthStart,companyYearEnd,companyMonthEnd) ) {
			...
			// ajax 后台访问 保存数据 
				$.ajax({
					type: 'POST',
					url: '/ajax/profile',
					data: { companyName : companyName,positionName:positionName,companyYearStart:companyYearStart,companyMonthStart:companyMonthStart,companyYearEnd:companyYearEnd,companyMonthEnd:companyMonthEnd},
					dataType: 'json',
					headers: {
					'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
					},
...
// console.log(isdelta(2,1,2,1));
	...
	//me 结束月
	// ye*12+me  -  ys*12+ms
	function isdelta(ys,ms,ye,me){

		if (ys&&ms&&ms&&ye&&me) {
			if((parseInt(ye)-parseInt(ys)) >= 0){

				var delta  = (parseInt(ye)*12+parseInt(me)) - (parseInt(ys)*12+parseInt(ms));
				if (delta > 0) {
					return delta;
				}
			}
		...
		return false;
            			...
            			<form class="projectForm">
	            			<table>
	            				...
	            				<tr>
	            					<td valign="top"></td> 
	            					<td colspan="3">
										<input type="submit" onclick="projectexperadd()" value="保 存" class="btn_profile_save">
						          		<a class="btn_profile_cancel" href="javascript:;">取 消</a>
	            					</td>
	            				</tr>
	            			</tbody></table>
				...
				<script>
				//项目经验
				function projectexperadd(){
					var projectName = $("input[name='projectName']").val();
					var thePost = $("input[name='thePost']").val();
					var projectYearStart = $("input[name='projectYearStart']").val();
					var projectMonthStart = $("input[name='projectMonthStart']").val();
					var projectYearEnd = $("input[name='projectYearEnd']").val();
					var projectMonthEnd = $("input[name='projectMonthEnd']").val();
					var projectDescription = $("input[name='projectDescription']").val();
					if (projectName && thePost && projectYearStart && projectMonthStart &&projectYearEnd &&projectMonthEnd) {
						if ( isdelta(projectYearStart,projectMonthStart,projectYearEnd,projectMonthEnd) ) {
							//录入消息正确
							
							// ajax 后台访问 保存数据 
								$.ajax({
									type: 'POST',
									url: '/ajax/projectexper',
									data: { projectName : projectName,thePost:thePost,projectYearStart:projectYearStart ,projectMonthStart:projectMonthStart,projectYearEnd:projectYearEnd,projectMonthEnd:projectMonthEnd,projectDescription:projectDescription},
									dataType: 'json',
									headers: {
									'X-CSRF-TOKEN': $('meta[name="_token"]').attr('content')
									},
            ...
            <div class="content_r">
				...
				<div class="greybg qrcode mt20">
                	<img width="242" height="242" alt="拉勾微信公众号二维码" src="{{asset('/styles/images/qr_resume.png')}}">
...
<div style="display:none;">
	...
	<div class="popup" id="uploadFile">
	    <table width="100%">
	    	...
	    	<tr>
	        	<td align="center"><img width="55" height="16" alt="loading" style="visibility: hidden;" id="loadingImg" src="{{asset('/styles/images/loading.gif')}}"></td>
	        </tr>
	    </tbody></table>
	</div><!--/#uploadFile-->
...
<script src="/styles/js/profile.min.js" type="text/javascript"></script>
...
<script src="{{asset('/styles/js/core.min.js')}}" type="text/javascript"></script>
